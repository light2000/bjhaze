...
use Closure, DateTime, ArrayAccess, PDO, finfo;
...
class Validator
{
    ...
    public function validateUnique($value, $parameters)
    {
        ...
        foreach ($params as $k => $value)
            $statement->bindParam($k + 1, $value, is_int($value) ? PDO::PARAM_INT : (is_null($value) ? PDO::PARAM_NULL : PDO::PARAM_STR));
        $statement->execute();
    ...
    {
        return $this->validateIn(pathinfo($value, PATHINFO_EXTENSION), array(
            'jpg',
            'jpeg',
            'png',
            'gif',
            'bmp'
        )) && $this->validateMimes($value, array(
            'image/jpeg',
            'image/png',
            'image/gif',
            'image/bmp'
