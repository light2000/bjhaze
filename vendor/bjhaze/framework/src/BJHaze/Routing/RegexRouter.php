...
class RegexRouter implements RouterInterface
{
    ...
    {
        return $this->forward($request->getPathInfo(), $request->getBaseUrl(true), 
                $this->getFrom($request)) ?  : $this->missingAction();
    }
    ...
    protected function matches ($pattern, $url)
    {
        $pattern = self::REGEX_DELIMITER . '^' .
                 self::complie($pattern, (array) $this->variablePatterns, 
                        '[^' . preg_quote($this->separator) . ']++', "(?P<%s>%s)") . '$' .
     ...
     * @return string
     */
    protected static function complie ($pattern, array $params, $def = null, $format = null)
    {
        return preg_replace_callback(self::REGEX_DELIMITER . '{(\w+)}' . self::REGEX_DELIMITER, 
                function  ($matches) use( $params, $def, $format)
                {
                    $var = isset($params[$matches[1]]) ? $params[$matches[1]] : $def;
                    if (null === $var)
                        throw new \LogicException('Parameter in pattern not found');
    ...
    public function buildUrl ($action, array $params = null)
    {
        foreach ($this->rules as $rule)
            if ($action == $rule['action'])
                return $params ? self::complie(
                        ! empty($rule['baseUrl']) ? $rule['baseUrl'] . $rule['path'] : $rule['path'], 
                        $params) : $rule['path'];
    }
